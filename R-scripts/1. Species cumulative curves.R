...
# --------------------------------------------------------------------------------------------
#Species Accumulation Curve for the current fish data 2013,2014,2016 (Sites M4,M7, M9) 
# --------------------------------------------------------------------------------------------
...
# -----------------------------
sites_keep <- c("M4","M7","M9")  # this dataset only has 3 sites
years_keep <- c(2013,2014,2016)
...
# ---- Plot: Species Accumulation Curve (2013–2016) ----
# Force whole-number ticks on both axes.
max_sites <- max(acc_df$sites, na.rm = TRUE)
max_rich  <- ceiling(max(acc_df$upper, na.rm = TRUE))
...
ggplot(acc_df, aes(x = sites, y = richness)) +
  ...
  geom_point(size = 2) +
  scale_x_continuous(
    breaks = seq(1, max_sites, by = 1),
    labels = number_format(accuracy = 1),
    limits = c(1, max_sites),
    expand = c(0, 0)
  ) +
  scale_y_continuous(
    breaks = seq(0, max_rich, by = 1),
    labels = number_format(accuracy = 1),
    limits = c(0, max_rich),
    expand = c(0, 0)
  ) +
